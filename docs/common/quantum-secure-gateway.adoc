...
OpenShift Service Mesh 3.1 does not deliver an istio-proxy image with built-in support for Post-Quantum Cryptography. To enable post-quantum safe algorithms, you need to configure the https://github.com/open-quantum-safe/oqs-provider[OQS (Open Quantum Safe) provider] in the proxy container.
...
* `accessLogFile: /dev/stdout`: Enables access logging for debugging and monitoring
* `ecdhCurves: [X25519MLKEM768]`: Configures the mesh to use the hybrid post-quantum key exchange algorithm
